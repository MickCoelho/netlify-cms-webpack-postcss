...
export default (...args) => (Component) => {
  ...
  const ConnectedComponent = connect.apply(null, connectArgs)(Component); //  eslint-disable-line

  const ComponentWithRematch = (props = {}) => {
    const { store, initialProps, initialState, ...others } = props; //  eslint-disable-line
